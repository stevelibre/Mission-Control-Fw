...
Init_USART2();
printf("USART 2 init ok. \n");

  while(1){
    //uart_putchar ('a');
    uart_write(2, "Hello", 5*sizeof(char));
    vTaskDelay(100);
    uart_write(2, "UART\n", 5*sizeof(char));
  //uart_putchar('b');
  }
  ...
  STM_EVAL_LEDOff(LED6);
    ...
    if (accelQueueStatus == pdPASS) {
      ...
      if(accel_trans_buffer != NULL) {
        snprintf(accel_trans_buffer,TRANS_BUFFER_LENGTH -1,"a %d, %d, %d \n",pvarx, pvary, pvarz);
      }
...
//    }
//  }else{
//      trace("ERROR receiving from the compass queue\n");
//  }
